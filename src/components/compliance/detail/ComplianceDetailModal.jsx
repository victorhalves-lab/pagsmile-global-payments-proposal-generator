  ...
  return (
    <div className="flex items-start gap-3 py-2.5 border-b border-white/[0.04] last:border-0">
      <div className={`w-5 h-5 rounded-full flex items-center justify-center shrink-0 mt-0.5 ${value ? 'bg-red-500/20' : 'bg-green-500/20'}`}>
        <span className={`text-[10px] font-bold ${value ? 'text-red-400' : 'text-green-400'}`}>{value ? '!' : '✓'}</span>
      </div>
      <div className="flex-1 min-w-0">
        <p className="text-white/70 text-xs leading-relaxed">{question}</p>
        {value && detail && <p className="text-yellow-400/70 text-xs mt-1 bg-yellow-500/5 p-2 rounded">{detail}</p>}
      </div>
...
const SIDEBAR_TABS = [
  ...
  { id: 'ubos', label: 'UBOs / Sócios', icon: Users },
  { id: 'directors', label: 'Diretores', icon: UserCheck },
  { id: 'contacts', label: 'Contatos', icon: Phone },
  { id: 'application', label: 'Aplicação', icon: MapPin },
  { id: 'compliance', label: 'Compliance', icon: ShieldCheck },
  { id: 'certification', label: 'Certificação', icon: Send },
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-3 mt-1">
                ...
                {data.registered_country && (
                  <span className="text-white/30 text-xs flex items-center gap-1"><Globe className="h-3 w-3" /> {data.registered_country}</span>
                )}
                {data.estimated_monthly_volume_usd > 0 && (
                  <span className="text-white/30 text-xs flex items-center gap-1"><DollarSign className="h-3 w-3" /> US$ {data.estimated_monthly_volume_usd.toLocaleString('pt-BR')}</span>
                )}
              ...
              <div>
                ...
                <div className="bg-white/[0.02] border border-white/[0.06] rounded-xl p-5">
                  <div className="grid grid-cols-2 gap-x-6 gap-y-4">
                    ...
                    {data.physical_office_address && <DetailField label="Endereço Físico" value={data.physical_office_address} fullWidth />}
              ...
              <div>
                ...
                {data.ubos?.length > 0 ? (
                  ...
                  <div className="bg-white/[0.02] border border-white/[0.06] rounded-xl p-8 text-center">
                    <p className="text-white/30 text-sm">Nenhum UBO informado</p>
                  </div>
                )}
              </div>
            )}
                ...
                {data.directors_same_as_ubos ? (
                  <div className="bg-white/[0.02] border border-white/[0.06] rounded-xl p-5">
                    <div className="flex items-center gap-2">
                      <div className="w-5 h-5 rounded bg-[#2bc196]/20 flex items-center justify-center"><span className="text-[#2bc196] text-[10px]">✓</span></div>
            ...
            {activeTab === 'contacts' && (
              <div>
                ...
                <div className="grid grid-cols-1 gap-2">
                  <ContactBlock title={t('compliance.accounting')} name={data.accounting_contact_name} email={data.accounting_contact_email} extra={data.accounting_invoice_address} />
                  <ContactBlock title={t('compliance.support')} name={data.support_contact_name} email={data.support_contact_email} extra={data.support_contact_phone} />
                  <ContactBlock title={t('compliance.complianceContact')} name={data.compliance_contact_name} email={data.compliance_contact_email} />
                </div>
              </div>
            )}
